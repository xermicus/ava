...
/// Read no further than this, so an endless file cannot stall the verifier.
const READ_LIMIT: u64 = 1 << 12;
...
impl crate::Game for SanityCheck {
    ...
    fn verify(
        ...
        let contents = match crate::read_at_most(&path, READ_LIMIT) {
            Ok(Some(contents)) => contents,
            Ok(None) => {
                return Ok(crate::failed(format!(
                    "{SUBMISSION_FILE} is over {READ_LIMIT} bytes"
                )));
            }
