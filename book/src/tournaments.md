...
`ava tournament -n <name> -g <game> -t <seconds> -c <combats>` opens a tournament, or the tournaments page of the web interface does. The game, the seconds every run is given and the combats every fight plays, 5 unless chosen otherwise, are fixed at that point, and so is the analyst: `--analyst harness/model[/thinking]`, or the analyze checkbox of the form, has every run of a round analyzed the moment it is over, the runs of the seats and the runs of the attacks alike, in parallel with what the round still plays and under the same cap. A failed analysis fails nothing, the run page offers it again.
