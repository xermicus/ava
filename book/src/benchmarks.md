...
- Live runs show their pushes and whether one passed, and their requests, tokens and cost aggregated from what the proxy has logged so far, which trails the run by the request in flight. Their points appear once the run is over, since the entries stay in the scoring container until then.
