...
The agents page names agents: its form adds one to `agents.json` with its harness, its model at the backend serving it, one choice per route of the registry and only the routes the harness speaks, and whether it is the analyst, the edit link beside an agent changes any of that, the remove button drops it. The file is written whole. The page lists the agents of the file, each behind its avatar, a grid of cells lit by a hash of harness and model, so the same agent looks the same everywhere whatever it is named.
