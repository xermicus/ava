...
- Every harness compacts within the `context_window` of its route: pi, opencode and codex read it from their configuration, claude from `CLAUDE_CODE_MAX_CONTEXT_TOKENS` and `CLAUDE_CODE_AUTO_COMPACT_WINDOW`, floored at 100000, with `CLAUDE_CODE_DISABLE_1M_CONTEXT` on a gateway. Codex caps a model outside its catalog at 272000. The run records the compactions the harness reports on its console and the largest context any of its requests carried, shown on the run page against the window and in the standings of a tournament per seat.
