...
`ava analyze -r <run> -a <agent> -m <model>` starts the agent on `runs/<run>` and this book, both mounted read only, with the analysis prompt as its task. The analyst plays in the sandbox a run plays in, with a home of its own and without a scoring container, and takes turns on its session the way a run does until it has written its summary and its analysis or its seconds are spent. The two files are assembled into `analysis.json` in the run directory together with the analyst, its harness version, the image, the seconds, the turns and the metrics of its requests, next to the analyst's console `analysis.log` and the access log of its proxy `analysis.access.log`; a failed analysis records its reason there instead. The analyses count towards the recorded usage of the backends. The run page shows the summary with the analysis folded behind it, both rendered from their markdown, and offers the same behind a button; the runs table says next to the state of every run whether it is being analyzed, was analyzed, or its analysis failed. `-e` and `-t` set the thinking level and the seconds, 900 by default, which every analysis that ever succeeded fits inside. The start panel of the web interface can have a run analyzed the moment it is over, by claude on claude-sonnet-5 at medium thinking for 900 seconds unless chosen otherwise, and the run shows as analyzing meanwhile.
