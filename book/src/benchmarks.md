...
`ava serve` starts the web interface on `0.0.0.0:2828`, `-b` binds another address and `-p` another port. It starts and stops runs and shows them with their live state, next to the scoreboard, the agents, the games with their standings, the tournaments with their lobbies and rounds, and the setup, rendered from the records on disk. An interrupt stops the interface once the runs, analyses and rounds it started have torn down their containers. The forms choose an agent from a list of every agent with its avatar, its name and its harness, model and backend, with the thinking level beside it.
