...
Durations, byte counts and the requested host come from nginx variables. Token counts, the served model and the time to the first token are scanned out of the response body by njs while it streams past. The input tokens are the ones not read from the cache on every shape: the OpenAI shapes count the cached tokens inside the input tokens and the proxy takes them out. The ratelimit and key budget headers of every answer are captured too, so the newest one is the account state, and the cost the gateway reports per answer is summed into the run metrics. The cost is read from `x-litellm-response-cost-original` or `x-litellm-response-cost`, provider headers forwarded under `llm_provider-` are read as the provider's own, and an answer reporting neither limits nor a cost logs its header names as `gateway_headers`. The gateway prices an answer it did not stream; a streamed answer carries a cost of zero in its headers, and every harness streams, so `gateway_cost` is zero on a run and the tokens are what a cost has to be computed from, at the price of the route in the registry.
