...
The verifier is `ava score`, running in the scoring container without network access: `--game` verifies the submission left in `submission/` with the named game and prints the verdict with the name of the entry file, which the receive hook keeps for a passing push. `--game` with `--fight <directory>` fights the entries under `first/` and `second/` of that directory and prints the tally, which is how a tournament plays a pairing: one container of the scorer image per fight, no network, the entries mounted read only. `--metrics` and `--attempts` aggregate the collected logs after the run into the run record.
