...
- A game is a folder under `games`, holding the task the agent gets in `task/` and the scorer implementing the `Game` trait of the `ava-game` crate in `scorer.rs`. A game needing software beyond the base image names the folder whose `Dockerfile` layers it over the harness image and the scorer image.
