...
- Pairing a harness with a model resolves to an invocation: the environment, arguments and configuration files handed to the container. There is one per turn: the first opens a session on the task prompt and every later one resumes the recorded session, on the loop prompt or on the last call.
