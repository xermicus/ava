...
- A game is a folder under `games`, holding the task the agent gets in `task/`, or in `defend/` and `attack/` for a multiplayer game agents attack, and the verifier implementing the `Game` trait of the `ava-game` crate in `scorer.rs`. A game needing software beyond the base image names the folder whose `Dockerfile` layers it over the harness image and the scorer image.
...
- The tournament record is `tournaments/<name>/tournament.json`: the game and its version at creation, the pairing scheme, the seconds every run is given, the combats every fight plays, one for a record from before they were fixed, the analyst and its seconds when one was chosen, the lobby as the agent in every seat, and the rounds. A round holds when it started and finished, one entry per seat naming the run it played and the seconds of the attempt whose entry fights, and the pairings in the order they were fought, each two seats, the second they were fought at, their tally, the reason when the tally is not the outcome of a fight, and the run that played the pairing for an attack an agent plays. A game whose entries stand alone records no pairings, since the standings compare the entries themselves. A round without an end is playing or broke off, and only finished rounds count for the standings.
